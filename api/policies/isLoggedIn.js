...
module.exports = function (req, res, next) {
  ...
  if (req.header("authorization")) {
    ...
    if (!token) {
      return res
        .status(401)
        .send({ messages: "Please logged in before continue" });
    }
    // If there is something, attempt to parse it as a JWT token
    return jwt.verify(token, sails.config.jwtSecret, (err, payload) => {
      // If there's an error verifying the token (e.g. it's invalid or expired),
      // redirect to the login page.
      if (err) {
        return res.status(401).send({
          messages: "Token it's invalid or expired",
        });
      }
      ...
      User.findOne(payload.user, (err, user) => {
        ...
        if (!user) {
          ...
          return res
            .status(401)
            .send({ messages: "Email or password is incorrect" });
        }
